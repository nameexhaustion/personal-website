...
First, we curry the `factorial` function:

```javascript
const factorial = (fn) => (v) => {
  if (v == 0) {
    return 1;
  }
...
const f = factorial(factorial);

f(7); // = 5040
```

Here, `factorial` <span style={{color: 'limegreen'}}>"is a function that binds what you pass it to the value `fn`, and then returns another function that contains calls to `fn`."</span>

What we pass `factorial` is a copy of itself, since we need the function that it returns to have access to a copy of itself. So `fn` becomes a copy of `factorial`.

Inside of this other function, we see it calling `fn(fn)(v - 1)`. Why the call to `fn(fn)` instead of directly calling `fn` by itself?

Since `fn` is a copy of `factorial` it also does the same thing as `factorial`.
...
Since `fn` is the same as `factorial`, then the way we use it also has to be the same - by passing it into itself!

The next (and final) step is trickier, and also quite long to explain.
We want to make it so that the can in fact "directly call `fn`" inside of the function that `factorial` returns like this:
...
Let's reason our way through this. We want to call `factorial` with "something" - let's name this `S`.
`S` needs to be a function that satisfies a certain property:

- Calling `S` with some argument needs to be equivalent to calling the result of calling `factorial` with `S` with some argument. i.e.

```latex
S(v) \xleftrightarrow{} factorial(S)(v)
```

From here, you can directly construct such an `S` from the above requirement:

```javascript
const S = (v) => factorial(S)(v);
```

We'll allow direct self-reference for now to make it easier to work with.

You can see that calling `S` is equivalent to calling the result of calling `factorial` with `S`.
...
- Calling `factorial` with "something" gives us a function. We'll once again name this "something" `S`
- `S` needs to call factorial with another "something" - we'll name this other "something" `T`
...
// i.e.
const S = factorial((v) => S(v));
```

You may wonder why `T` can't directly call `S`, i.e. `const T = S(v)`, or `const S = factorial(S(v))`.
The is because when javascript tries to evaluate what the value of `S` is, it will call `factorial` with `S(v)`,
which calls `S` with `v`, which then again calls `factorial` with `S(v)` and so on.. leading to an infinite loop.
...
We've already seen how to do this with the factorial function, and this is no different.

First, we'll curry `S`, and replace all calls to `S` from inside itself with `fn(fn)`:

```javascript
const S = (fn) => {
  return factorial((v) => fn(fn)(v));
};
```
...
First, we convert variable names (`fn` to `x`) and convert to an arrow function:

```javascript
const S = (x) => factorial((v) => x(x)(v));
```

Converting to an [IIFE](https://www.google.com/search?q=immediately+invoked+function+expression):

```javascript
const S = ((x) => factorial((v) => x(x)(v)))((x) => factorial((v) => x(x)(v)));
```

The above is `((x) => factorial((v) => x(x)(v)))` repeated twice.
...
Finally, instead of only working on the `factorial` function, we curry this and replace `factorial` with the argument `f`:

```javascript
const S = (f) => ((x) => f((v) => x(x)(v)))((x) => f((v) => x(x)(v)));
```
...
So now, for any function in the same format as our previous factorial function, `S(f)` enables recursion for it.

##### The Z combinator

`S` is actually named the Z combinator, which is a form of the Y combinator adapted to work with eagerly evaluated languages.
To see this, it helps to know some python.
We'll rename `S` to `Z` and convert it to equivalent python code.
...
Finally, the Y combinator is a more "relaxed" Z combinator that doesn't define the extra <tex>\lambda v</tex> in the body:
...
Hopefully, I've managed to provide an understandable explanation on the Y combinator.
